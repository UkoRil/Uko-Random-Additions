...
public class Rubyarmor extends ItemArmor {
    ...
    public Rubyarmor(ItemArmor.ArmorMaterial material, int renderIndex, int armorType) {
    ...
    public String getArmorTexture(ItemStack stack, Entity player, int slot, String type) {
        if (this.armorType == 2) {
            return "ura:textures/models/armor/ruby_layer_2.png";
        } else {
            return "ura:textures/models/armor/ruby_layer_1.png";
        }
    }
...
public void onArmorTick(World world, EntityPlayer player, ItemStack itemStack) {
    if (!world.isRemote) {
        ...
        if (helmet != null && chest != null &&
            helmet.getItem() instanceof Rubyarmor &&
            chest.getItem() instanceof Rubyarmor) {
            player.addPotionEffect(new PotionEffect(Potion.moveSpeed.id, 40, 0, true));
        }

        if (legs != null && boots != null &&
            legs.getItem() instanceof Rubyarmor &&
            boots.getItem() instanceof Rubyarmor) {
